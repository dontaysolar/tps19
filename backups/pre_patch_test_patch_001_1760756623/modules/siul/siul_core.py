...
import os, json, sqlite3, threading, time, hashlib, uuid
...
class SIULCore:
    ...
    def process_unified_logic(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Use high-entropy unique chain id to avoid UNIQUE constraint collisions
            chain_id = f"siul_{int(time.time()*1000)}_{uuid.uuid4().hex[:8]}"
